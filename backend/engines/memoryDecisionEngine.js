...
const colorPairs = new Set(['black:acid','black:ivory','black:olive','burgundy:navy','charcoal:ivory','charcoal:moss','charcoal:sky','denim:tobacco','ivory:navy','navy:oatmeal'])
const pairingCategories = { tops:['bottoms','outerwear','shoes'], bottoms:['tops','outerwear','shoes'], outerwear:['tops','bottoms','shoes'], shoes:['tops','bottoms','outerwear'], accessories:['tops','outerwear','shoes'] }
const requiredOutfitCategories = { tops:['bottoms','shoes'], bottoms:['tops','shoes'], outerwear:['tops','bottoms','shoes'], shoes:['tops','bottoms'], accessories:['tops','bottoms','shoes'] }
...
function compatibleWardrobeItems(product, wardrobe) {
  ...
  return wardrobe
    ...
    .map((item) => {
      ...
      const score = 8 + color.score + (sceneOverlap ? 24 : 5) + (item.liked !== false ? 8 : 0) + (reliableWear ? 10 : item.wearFrequency === 'monthly' ? 4 : 0) + (preferredCategories.includes(item.category) ? 10 : 0)
      return { item, score:clamp(score), signals }
    })
...
function buildOutfitPlans(product, wardrobe) {
  ...
  const grouped = Object.fromEntries(planCategories.map((category) => [category, candidates
  ...
  for (let planIndex = 0; planIndex < 3; planIndex += 1) {
    ...
    plans.push({
      ...
      confidence:clamp(averageScore * .58 + pairAverage * .42 + coverage * 22),
      ready:missingCategories.length === 0,
      missingCategories,
      items:selected.map(({item,score,signals}) => ({ id:item.id,name:item.name,category:item.category,color:item.color,fit:item.fit,score,signals })),
    })
  }
  return plans
}
...
function buildUsagePlan(product, useFrequency, monthlyBudget, outfitPlans) {
  ...
  return { expectedWearsPerMonth, costPerWear12m:price ? Math.round((price / annualWears) * 10) / 10 : 0, budgetShare:Math.round(price / Math.max(1, monthlyBudget) * 100), completeOutfitCount:outfitPlans.filter((plan) => plan.ready).length, draftOutfitCount:outfitPlans.length }
}

function buildNextActions({ verdict, gates, missing, outfitPlans, wardrobe, similarItems }) {
  const actions = []
  const add = (code, extra = {}) => { if (!actions.some((action) => action.code === code)) actions.push({ code, ...extra }) }
...
export function analyzeMemoryPurchase(product = {}, memory = {}, wardrobe = []) {
  ...
  const styleMatches = styleRecords.filter((record) => { const style = styleById[record.value]; const terms = style ? [style.name,style.chineseName,...style.keywords].filter(Boolean).map((term) => String(term).toLowerCase()) : []; return terms.some((term) => keywords.includes(term)) })
  ...
  const preferenceDelta = colorSuccess.reduce((sum,record) => sum + record.confidence * 16,0) - colorFailure.reduce((sum,record) => sum + record.confidence * 30,0) + categorySuccess.reduce((sum,record) => sum + record.confidence * 12,0) - categoryFailure.reduce((sum,record) => sum + record.confidence * 22,0) - avoidConflicts.reduce((sum,record) => sum + record.confidence * 26,0)
  const styleFit = clamp(48 + styleMatches.reduce((sum, record) => sum + record.confidence * 28, 0) + (keywords ? 8 : 0) + preferenceDelta)
  ...
  const wardrobeCompatibility = clamp((activeWardrobe.length ? 38 : 24) + compatibleAverage * .48 + Math.min(14, compatibleItems.length * 5) - duplicateRisk * .32)
  ...
  const budgetFit = clamp(100 - Math.max(0, price / Math.max(1, monthlyBudget) - .3) * 85)
  ...
  return { verdict, totalScore, dimensions:{ styleFit, fitConfidence, wardrobeCompatibility, useFrequency, budgetFit, duplicateRisk, idleRisk }, gates, evidence:[{code:'styleEvidence',score:styleFit,confidence:styleRecords.length ? Math.max(...styleRecords.map((record) => record.confidence)) : .2,count:styleRecords.length + colorSuccess.length + colorFailure.length + categorySuccess.length + categoryFailure.length + avoidConflicts.length},{code:'fitEvidence',score:fitConfidence,confidence:fitSuccess[0]?.confidence || fitFailure[0]?.confidence || .2,count:fitSuccess.length + fitFailure.length},{code:'wardrobeEvidence',score:wardrobeCompatibility,confidence:activeWardrobe.length ? .75 : .15,count:activeWardrobe.length,similarCount:similarItems.length},{code:'sceneEvidence',score:useFrequency,confidence:sceneRecords.length ? .7 : .25,count:sceneRecords.length},{code:'budgetEvidence',score:budgetFit,confidence:.98,monthlyBudget}], reasons, missing, similarItems:similarItems.slice(0,5).map(({item,score}) => ({id:item.id,name:item.name,score})), compatibleItems, outfitPlans, usagePlan, nextActions, whatChanges, memorySnapshot:{recordCount:records.length,wardrobeCount:activeWardrobe.length,monthlyBudget} }
}
