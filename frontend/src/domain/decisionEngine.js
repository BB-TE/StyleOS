...
export function analyzePurchaseWithMemory(product, memory, wardrobe = []) {
  ...
  const styleMatches = styleRecords.filter((record) => {
    ...
    return style && (keywords.includes(style.name.toLowerCase()) || style.keywords.some((keyword) => keywords.includes(keyword)))
  })
  const styleFit = clampScore(48 + styleMatches.reduce((sum, record) => sum + record.confidence * 28, 0) + (keywords ? 8 : 0))
  ...
  const wardrobeCompatibility = clampScore(72 - duplicateRisk * .38 + Math.min(24, (product.scenes || []).length * 8))
  ...
  const budgetFit = clampScore(100 - Math.max(0, Number(product.price || 0) / Math.max(1, monthlyBudget) - .3) * 85)
  ...
  const evidence = [
    { code: 'styleEvidence', score: styleFit, confidence: styleRecords.length ? Math.max(...styleRecords.map((record) => record.confidence)) : .2, count: styleRecords.length },
    ...
    { code: 'wardrobeEvidence', score: wardrobeCompatibility, confidence: wardrobe.length ? .75 : .15, count: wardrobe.length, similarCount: similarItems.length },
  ...
  return {
    ...
    whatChanges,
    memorySnapshot: { recordCount: records.length, wardrobeCount: wardrobe.length, monthlyBudget },
  }
}
